...
| POST | `/v1/system-configuration-administrative-plan/initiate` | Initiate |
| GET | `/v1/system-configuration-administrative-plan` | Retrieve (list) |
| GET | `/v1/system-configuration-administrative-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/system-configuration-administrative-plan/{crId}/update` | Update |
| PUT | `/v1/system-configuration-administrative-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/system-configuration-administrative-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-systems-administration:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-systems-administration ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-systems-administration` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
